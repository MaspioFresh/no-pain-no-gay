...
export function Dashboard({ plans, unit, onToggleUnit, onStartPlan, onViewPlan, onModifyPlan, onDeletePlan, onAddPlan, onCreatePlan, onViewHistory, onViewProgress, onManageData }: DashboardProps) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (imported.type === 'single_plan' && imported.plan) {
          // Pass to parent via a new prop or just call existing onAddPlan
          onAddPlan?.(imported.plan);
          alert(`Scheda "${imported.plan.name}" importata!`);
        } else {
          alert('Formato file non valido per una singola scheda.');
        }
      } catch (err) {
    ...
    <div className="flex flex-col space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col space-y-2">
        <div className="flex justify-between items-start">
          ...
          <button 
            ...
            className="hardware-card px-3 py-1 text-[10px] mono-label flex items-center space-x-2 bg-transparent border border-accent text-accent"
          >
            <Settings2 size={12} className="accent-text" />
            <span>UNITS: {unit.toUpperCase()}</span>
          </button>
        </div>
        <p className="mono-label underline decoration-accent/30 underline-offset-4">Piattaforma di addestramento professionale</p>
      </header>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          ...
          <span className="mono-label text-accent font-bold text-[10px]">Dati</span>
        </button>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <label className="flex items-center space-x-1 px-3 py-1.5 bg-transparent rounded-full border border-accent text-accent text-[10px] mono-label cursor-pointer hover:bg-accent/10 active:scale-95 transition-all">
      ...
      <div className="hardware-card p-6 border-accent/20 bg-accent/5">
        <div className="flex items-center space-x-3 mb-2">
          <div className="w-2 h-2 rounded-full bg-accent animate-pulse" />
          <span className="mono-label text-accent">Stato Sistema</span>
        </div>
        <p className="text-sm text-white/70">
          Tutti i moduli sono operativi. L'allenamento quotidiano è consigliato per una crescita ottimale.
        </p>
      </div>
    </div>
  );
